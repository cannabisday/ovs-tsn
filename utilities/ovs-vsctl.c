...
//1015
//extern struct shash port_speed_map;  // port_speed_map 전역 변수 참조
...
cmd_add_br(struct ctl_context *ctx)
{
    ...
    int vlan;
    int udp_server_port;  // joo) added port num
    ...
    vsctl_context_invalidate_cache(ctx);
    // 브리지 이름에서 UDP 서버 포트 번호 추출
    if (strlen(br_name) >= 4 && strncmp(br_name, "br", 2) == 0) {
        char bridge_num_str[3] = { br_name[2], br_name[3], '\0' };  // "br03"에서 "03" 추출
        udp_server_port = 20000 + atoi(bridge_num_str);  // UDP 포트 번호는 "200" + "03" = 20003
    } else {
        ctl_fatal("Invalid bridge name format: %s. Expected format 'brXX'.", br_name);
    }

    // UDP 서버 시작 (추출된 포트를 사용)
...
char client_ip[INET_ADDRSTRLEN]; // IPv4 주소 문자열을 저장하기 위한 버퍼
         ...
         char *settings[], int n_settings)
{
    ...
    //client) int udp_client_port = 0; // 클라이언트 포트를 위한 변수 추가

    uint32_t new_speed = 10; //psm


    // joo) 로컬 인터페이스 IP 주소 가져오기
    ...
    printf("ovs_vsctl) port: %s, host_ip: %s \n", port_name, hostip);//(*ifaces)->name, n_ifaces);
    ...
    if (strlen(port_name) == 6 && port_name[0] == 'p' && port_name[3] == 't' && port_name[4] == '0' && port_name[5] == '0') {
        ...
        // 백그라운드에서 실행을 위해 fork() 호출
        pid_t pid = fork();
        if (pid < 0) {
            perror("Failed to fork process");
            exit(EXIT_FAILURE);
        } else if (pid > 0) {
            // 부모 프로세스는 종료하여 터미널 점유를 해제
            printf("ovs_vsctl)UDP server started in the background with port %d\n", udp_server_port);
            printf("port is inserted 0) \n");
            // 이후 OVS 관련 작업 수행
            if (bridge->parent) {
            ...
            for (i = 0; i < n_settings; i++) {
                char *error = ctl_set_column("Port", &port->header_, settings[i], ctx->symtab);
                if (error) {
                    ctl_fatal("%s", error);
                }
            }
        ...
        // 자식 프로세스에서 실행될 부분
        // 자식 프로세스는 setsid()로 새로운 세션을 생성하여 독립된 데몬처럼 실행되게 합니다.
        if (setsid() < 0) {
            perror("Failed to create new session");
            exit(EXIT_FAILURE);
        }
        printf("ovs_vsctl)port is inserted 1) \n");
        ...
        // 자식 프로세스에서 UDP 소켓 서버를 새로 생성하여 포트 바인딩
        int sockfd;
        ...
        printf("port is inserted 2) \n");

        // UDP 서버 설정
        memset(&server_addr, 0, sizeof(server_addr));
        server_addr.sin_family = AF_INET;
        server_addr.sin_addr.s_addr = INADDR_ANY; // 모든 인터페이스에서 수신하도록 변경
        ...
        printf("ovs_vsctl)Socket bound to port %d successfully.\n", udp_server_port);
        printf("port is inserted 3) \n");
        ...
        while (1) {
            ...
            if (n < 0) {
                ...
                // 클라이언트에게 응답 전송
                snprintf(buffer, sizeof(buffer), "ACK: Received your packet");
                if (sendto(sockfd, buffer, strlen(buffer), 0, (struct sockaddr *)&client_addr, addr_len) < 0) {
                    perror("Failed to send ACK to client");
                } else {
                    printf("ovs_vsctl)Sent ACK to client IP: %s, Port: %d\n", client_ip, ntohs(client_addr.sin_port));
                }
            }
        }

        // 자식 프로세스는 백그라운드에서 종료하지 않음
        exit(EXIT_SUCCESS);
        printf("ovs_vsctl)port is inserted 4) \n");

    }/* else if (strlen(port_name) == 6 && port_name[0] == 'p' && port_name[3] == 't') {
        ...
        set_port_speed(port_name, new_speed);
        ...
        // pXXtYY 형식이 아닌 경우 처리
        printf("ovs_vsctl)Port %s does not match the format pXXtYY, skipping UDP client start.\n", port_name);
    }
    printf("ovs_vsctl)port is inserted 5) \n");
