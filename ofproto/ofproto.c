...
#include <unistd.h>
 ...
 // Header for udp_server.c    
#include "udp_client.h" // Header for udp_client.c
#include "bitmap.h"
#include "bundles.h"
...
/*
void update_port_speed() {
    uint32_t speed = get_port_speed("p04t03");
    printf("Port speed for p04t03: %u\n", speed);
}*/

/*void print_port_speed_map() {
    struct shash_node *node;

    SHASH_FOR_EACH(node, &port_speed_map) {
        struct port_speed_info *info = node->data;
        printf("Port: %s, Speed: %lu\n", node->name, info->speed);
    }
}
*/
...
ofproto_init(const struct shash *iface_hints)
{
    ...
    /* 최초 1회만 port_speed_map을 초기화 */
    
    if (!is_port_speed_map_initialized) {
        is_port_speed_map_initialized = true;

        init_port_speed_map();
        VLOG_INFO("Port speed map initialized once.");
    } else {
        VLOG_INFO("Port speed map already initialized.");
    }
...
ofproto_run(struct ofproto *p)
{
    ...
    //joo) called frequently
    //VLOG_WARN("1) ofproto_run start of ofproto.c");
    ...
    //joo esse)
    //VLOG_WARN("Value of new_seq: %"PRIu64"", new_seq);
    ...
    if (new_seq != p->change_seq) {
        ...
        SSET_FOR_EACH (devname, &devnames) {
                //joo)
            VLOG_WARN("up 02 start of ofproto.c");
            VLOG_WARN("updating prot: %s", devname);
    ...
    ovs_mutex_unlock(&ofproto_mutex);
    //joo) esse
    //VLOG_WARN("1) ofproto_run end of ofproto.c");
                 ...
                 ofp_port_t *ofp_portp)
{
    ...
    error = ofproto->ofproto_class->port_add(ofproto, netdev);
        //joo)
    //VLOG_WARN("up 03 start of ofproto.c");
    if (!error) {
...
reinit_ports(struct ofproto *p)
{
    ...
    SSET_FOR_EACH (devname, &devnames) {
            //joo)
    //VLOG_WARN("up 04 start of ofproto.c");
        update_port(p, devname);
    }
    sset_destroy(&devnames);
}
...
update_port(struct ofproto *ofproto, const char *name)
{
    ...
    int error = 0;
    //joo)psm
    /*uint32_t *speed_ptr = (uint32_t *)shash_find_data(&port_speed_map, "p04t03");
if (speed_ptr) {
    printf("Port speed: %u\n", *speed_ptr);
} else {
    printf("Port speed not found for p04t03\n");
}*/
    //1015VLOG_WARN("joo)-on Port %s speed is: %u\n", name, get_port_speed(name));


    char formatted_port_name[10] = ""; // pXXtYY 형식의 포트 이름 저장
    int bridge_num = 0, port_num = 0;
    ...
    /*
    VLOG_WARN("Physical port description: port_no=%u, hw_addr="ETH_ADDR_FMT", name=%s",
              pp.port_no, ETH_ADDR_ARGS(pp.hw_addr), pp.name);
    if (ofproto && ofproto->name) {
        VLOG_WARN("Bridge name from ofproto: %s", ofproto->name);
    } else {
        VLOG_WARN("Bridge name is not available.");
    }
    VLOG_WARN("Current netdev features: %u\n BF", pp.curr );
    */
    //uint32_t *speed_ptr = xmalloc(sizeof(uint32_t));

    //psm
    //print_port_speed_map();

    //uint32_t speed_a = get_port_speed("p04t03");
 ...
 // 포트 이름의 숫자 부분 추출
    ...
    VLOG_WARN("Bridge name: %s", ofproto->name);
    VLOG_WARN("Port name: %s", pp.name);
    // 새로운 pXXtYY 형식의 포트 이름 생성

    snprintf(formatted_port_name, sizeof(formatted_port_name), "p%02dt%02d", bridge_num, port_num);
    VLOG_WARN("Current speed: %u kbps BF", pp.curr_speed);
    VLOG_WARN("New port defined : %s ", formatted_port_name);
    ...
    if (netdev) {
        ...
        if (port) {
            VLOG_WARN("ofport name: %s", netdev_get_name(port->netdev));
            VLOG_WARN("ofport number: %u", port->ofp_port);
        } else {
            VLOG_WARN("No ofport found for name: %s", name);
        }
        ...
        if (port && !strcmp(netdev_get_name(port->netdev), name)) {
            ...
            //joo) this part is a triger to update port status
...
static enum ofperr
port_mod_start(struct ofconn *ofconn, struct ofputil_port_mod *pm, struct ofport **port)
{
    struct ofproto *p = ofconn_get_ofproto(ofconn);
...
static void
port_mod_finish(struct ofconn *ofconn, struct ofputil_port_mod *pm, struct ofport *port)
{
...
handle_port_mod(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    if (!error) {
        ...
        //joo) new
        ////uint32_t curr_speed = start_udp_client("127.0.0.1", 12345);
        //port->pp.curr_speed = curr_speed;
        VLOG_WARN("udp) Port %d Current Speed: %u\n", port->pp.port_no, port->pp.curr_speed);
        //joo) start_udp_client("127.0.0.1", 12345, port);
        printf("joo)_ofproto.c : Port num: %d Current Speed: %u\n", port->pp.port_no, port->pp.curr_speed);
    }
    return error;
}
