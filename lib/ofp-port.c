                          ...
                          struct ofp11_port *op)
{
    ...
    op->curr_speed = htonl(pp->curr_speed >> 2);
    VLOG_WARN("ofputil_encode_ofp11_port %u kbps", ntohl(op->curr_speed));
                      ...
                      struct ofputil_phy_port *pp)
{
    ...
    //joo)
    //VLOG_WARN("ofputtil_pull_phy_port start of ofp-port.c");
    switch (ofp_version) {
    case OFP10_VERSION: {
...
{
    //joo)
    //VLOG_WARN("ofputil_encode_port_status start of ofp-port.c");
