...
struct performance_data {
    char port_name[64];      // 포트 이름
    uint32_t curr_speed;     // 현재 속도 (kbps)
};

/* 전역 성능 데이터 및 락 선언 */
extern struct performance_data perf_data;      // 성능 데이터를 저장할 전역 변수
extern pthread_mutex_t perf_data_lock;         // 성능 데이터 접근을 위한 락
